...
export const MobileMenu = ({ menuOpen, setMenuOpen }) => {
  return (
    <div
      className={`fixed top-0 left-0 w-full z-40 bg-[rgba(10,10,10,0.8)] flex flex-col items-center justify-center duration-300 transition-all ease-in-out ${
        menuOpen
    ...
    >
        ...
        <a href="#projects" className={`text-2xl font-semibold text-white my-4 transform transition-transform duration-300 ${menuOpen ? "opacity-100 translate-y-0" : 'opacity-0 translate-y-5'}`} onClick={()=> setMenuOpen(false)}>Project</a>
        <a href="#certification" className={`text-2xl font-semibold text-white my-4 transform transition-transform duration-300 ${menuOpen ? "opacity-100 translate-y-0" : 'opacity-0 translate-y-5'}`} onClick={()=> setMenuOpen(false)}>Certificates</a>
